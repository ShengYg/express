...
def im_detect(net, im, feat_blob, boxes=None):
    ...
    # scores:[not_person, is_person]

def conv_feat(net, im, feat_blob, boxes=None):
    """get conv feat to visualize
    ...
    features = net.blobs[feat_blob].data.copy()     # 1 * 2048 * H * W
...
def test_net_on_gallery_set(net, imdb, feat_blob, max_per_image=100, thresh=0.05, vis=False):
...
def vis_conv(net, imdb, feat_blob, max_per_image=100, thresh=0.05, vis=False):

    num_images = 0  #[0, len(imdb.image_index) - 1]
    box_proposals = None

    im = cv2.imread(imdb.image_path_at(num_images))
    features = conv_feat(net, im, feat_blob, box_proposals)

    output_dir = get_output_dir(imdb)
    det_file = os.path.join(output_dir, 'conv_feat.pkl')
    with open(det_file, 'wb') as f:
