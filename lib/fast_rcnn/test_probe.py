...
def _get_blobs(im, rois):
    ...
    blobs['rois'] = _get_rois_blob(rois, im_scale_factors) #(1, 5) ==> (pyramid_level, bbox)
...
def test_net_on_probe_set(net, images, rois, feat_blob, output_dir):
    ...
    for i in xrange(num_images):
        ...
        _t['im_exfeat'].tic()
        feat = im_exfeat(net, im, roi, feat_blob) # roi is gt_bbox
        features.append(feat) # gt_features
        _t['im_exfeat'].toc()
