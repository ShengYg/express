...
from rpn_msr.proposal_layer_train import proposal_layer as proposal_layer_py_train
from rpn_msr.proposal_layer_test import proposal_layer as proposal_layer_py_test
...
class RPN(nn.Module):
    ...
    def __init__(self):
        ...
        self.features = VGG16_HALF(bn=False)
        self.conv1 = Conv2d(256, 512, 3, same_padding=True)
        self.score_conv = Conv2d(512, 18, 1, relu=False, same_padding=False)
        self.bbox_conv = Conv2d(512, 36, 1, relu=False, same_padding=False)
    ...
    def forward(self, im_data, im_info, gt_boxes=None, im_inds=None):
        ...
        rpn_cls_prob_reshape = self.reshape_layer(rpn_cls_prob, 18)
        ...
        if self.training:
            ...
            rpn_data = self.anchor_target_layer(rpn_cls_score, gt_boxes, im_inds,
    ...
    def proposal_layer(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, cfg_key, _feat_stride):
        ...
        x = None
        if cfg_key == 'TRAIN':
            x = proposal_layer_py_train(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, cfg_key, _feat_stride)
        else:
            x = proposal_layer_py_test(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, cfg_key, _feat_stride)
        x = network.np_to_variable(x, is_cuda=True)
        return x.view(-1, 5)

    @staticmethod
    def anchor_target_layer(rpn_cls_score, gt_boxes, im_inds, im_info, _feat_stride):
        """
        ...
        rpn_labels, rpn_bbox_targets, rpn_bbox_inside_weights, rpn_bbox_outside_weights = \
            anchor_target_layer_py(rpn_cls_score, gt_boxes, im_inds, im_info, _feat_stride)
...
class FasterRCNN(nn.Module):
    ...
    def __init__(self, classes=None, debug=False):
        ...
        self.fc6 = FC(256 * 7 * 7, 1024)
        self.fc7 = FC(1024, 1024)
    ...
    def forward(self, im_data, im_info, gt_boxes=None, im_inds=None):
        features, rois = self.rpn(im_data, im_info, gt_boxes, im_inds)

        if self.training:
            roi_data = self.proposal_target_layer(rois, gt_boxes, im_inds, self.n_classes)
            rois = roi_data[0]
    ...
    @staticmethod
    def proposal_target_layer(rpn_rois, gt_boxes, im_inds, num_classes):
        """
        ----------
        ...
        rois, labels, bbox_targets, bbox_inside_weights, bbox_outside_weights = \
            proposal_target_layer_py(rpn_rois, gt_boxes, im_inds, num_classes)
