...
class Conv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, relu=True, same_padding=False, bn=False):
        ...
        self.bn = nn.BatchNorm2d(out_channels, eps=1e-05, momentum=0.1, affine=True) if bn else None
...
def load_pretrain_net(fname, net, num=17*2):
    import h5py
    h5f = h5py.File(fname, mode='r')
    for k, v in net.state_dict().items()[:num]:
        param = torch.from_numpy(np.asarray(h5f[k[9:]]))
        v.copy_(param)
