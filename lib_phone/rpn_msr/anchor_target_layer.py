...
def anchor_target_layer(rpn_cls_score, gt_boxes, im_inds, im_info, _feat_stride=[16, ]):
    """
    ...
    for batch_image in range(batch_size):
        ...
        inds_inside = np.where(
            ...
            (all_anchors[:, 2] < im_info_i[1] + _allowed_border) &  # width
            (all_anchors[:, 3] < im_info_i[0] + _allowed_border)  # height
        )[0]
        ...
        # labels
        # pdb.set_trace()
        labels_all = labels_all.reshape((batch_size, height, width, A))
        labels_all = labels_all.transpose(0, 3, 1, 2)
        rpn_labels = labels_all.reshape((batch_size, 1, A * height, width)).transpose(0, 2, 3, 1)

        # bbox_targets
        bbox_targets_all = bbox_targets_all \
            .reshape((batch_size, height, width, A * 4)).transpose(0, 3, 1, 2)

        rpn_bbox_targets = bbox_targets_all
        ...
        rpn_bbox_inside_weights = bbox_inside_weights_all
        ...
        rpn_bbox_outside_weights = bbox_outside_weights_all
