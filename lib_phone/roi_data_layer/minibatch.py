...
def get_minibatch(roidb, num_classes):
    ...
    for i in range(num_images):
        ...
        im_inds_all.append(im_inds)
        im_info_all.append(np.array([[im_info[i][0], im_info[i][1], im_scales[i]]],dtype=np.float32))
        
    gt_boxes_all = np.vstack(gt_boxes_all)
    im_inds_all = np.vstack(im_inds_all)
    im_info_all = np.vstack(im_info_all)
