...
"""
roidb:
'boxes'         (selective_boxes * 4)
'gt_classes'    selective_boxes
'gt_overlaps'   (selective_boxes * num_classes) return overlaps_area
'flipped'
'seg_areas'
'bbox_targets'  selective_boxes * (class, dx, dy, dw, dh)
"""
import numpy as np
...
def prepare_roidb(imdb):
    ...
    sizes = [PIL.Image.open(imdb.image_path_at(i)).size
             for i in xrange(imdb.num_images)]      #[(w, h), ...]
    roidb = imdb.roidb
...
def add_bbox_regression_targets(roidb):
    ...
    if cfg.TRAIN.BBOX_NORMALIZE_TARGETS_PRECOMPUTED:
        ...
        means = sums / class_counts     # classes * 4
        stds = np.sqrt(squared_sums / class_counts - means ** 2)       # classes * 4
